...
namespace Sandbox.Lib
{
  public class UriString
  {
    ...
    {
      this.Protocol = FirstNonEmpty(uri.Protocol, this.Protocol);
      this.Host = FirstNonEmpty(uri.Host, this.Host);
      this.Verb = FirstNonEmpty(uri.Verb, this.Verb);

      // Path
      if (!string.IsNullOrEmpty(uri.Path))
      {
        var path = $"{uri.Path}{uri.Verb}";
        this.Path = uri.Path.StartsWith("/") ? path : $"{this.Path}/{path}";
      }
    ...
    private string FirstNonEmpty(params string[] terms)
    {
